...
void RegisterScreen::open() {
    app_.screen_ = App::Screen::Register;
...
void RegisterScreen::step(int delta) {
    constexpr std::array<Focus, 6> order{
        Focus::Username, Focus::Email, Focus::Password, Focus::AutoLogin,
        Focus::Submit, Focus::Back
    };
    int index = 0;
...
void RegisterScreen::backToWelcome() {
    password_.clear();
    app_.status_.clear();
...
void RegisterScreen::update(u32 keysDown, u32 keysHeld, circlePosition circle, touchPosition touch, bool touched) {
    ...
    if (BackButton.contains(touch)) {
        backToWelcome();
    } else if (UiRect{24.0F, 48.0F, 272.0F, 34.0F}.contains(touch)) {
        focus_ = Focus::Username;
        app_.requestText(username_, app_.localization_.get(TextId::Username), false);
    } else if (UiRect{24.0F, 88.0F, 272.0F, 34.0F}.contains(touch)) {
        focus_ = Focus::Email;
        app_.requestText(email_, app_.localization_.get(TextId::Email), false);
    } else if (UiRect{24.0F, 128.0F, 272.0F, 34.0F}.contains(touch)) {
        focus_ = Focus::Password;
        app_.requestText(password_, app_.localization_.get(TextId::Password), true);
    } else if (UiRect{24.0F, 166.0F, 272.0F, 26.0F}.contains(touch)) {
...
void RegisterScreen::render() {
    ...
    app_.drawCentered(app_.localization_.get(TextId::CreateAccount), 160.0F, 18.0F, 0.80F, Ink);
    ...
    const UiRect usernameField{24.0F + slide, 48.0F, 272.0F, 34.0F};
    ...
    const UiRect emailField{24.0F - slide, 88.0F, 272.0F, 34.0F};
    ...
    const UiRect pwField{24.0F + slide, 128.0F, 272.0F, 34.0F};
    ...
    const UiRect autoRect{24.0F, 166.0F, 272.0F, 26.0F};
